...
struct
    ...
    val (eagain, _) = _symbol "ML_EAGAIN" : (unit -> int) * (int -> unit) ;
    val (pf_inet, _) = _symbol "ML_PF_INET" : (unit -> int) * (int -> unit) ;
    val (sock_stream, _) = _symbol "ML_SOCK_STREAM" : (unit -> int) * (int -> unit) ;
...
(*            val cl = _import "close" : int -> int ;*)
    ...
    fun select (ins : sdesc list, outs : sdesc list, to) =
        let
            ...
            val (rd, _)   = _symbol "ml_poll_rd" : (unit -> Word16.word) * (Word16.word -> unit) ;
            val (wr, _)   = _symbol "ml_poll_wr" : (unit -> Word16.word) * (Word16.word -> unit) ;
            val (ex, _)   = _symbol "ml_poll_ex" : (unit -> Word16.word) * (Word16.word -> unit) ;
            val (hup, _)  = _symbol "ml_poll_hup" : (unit -> Word16.word) * (Word16.word -> unit) ;
            val (nval, _) = _symbol "ml_poll_nval" : (unit -> Word16.word) * (Word16.word -> unit) ; 
